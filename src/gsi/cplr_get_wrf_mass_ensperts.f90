...
130         format('Disabling ens_fast_read because number of ensemble members (',I0,') is greater than number of MPI ranks (',I0,').')
      ...
      ens_parallel_read: if(do_ens_fast_read) then
         ...
         ens_read_loop: do n=1,n_ens
            ...
            if(mype==iope) then
               ...
               bad_input=.false.
               if( do_radar )then
                 call this%parallel_read_wrf_mass_step1(filename,gg_ps,gg_tv,gg_u,gg_v,gg_rh,gg_w,&
                                                        gg_dbz,gg_qr,gg_qs,gg_qi,gg_qg,gg_qnc,&
                                                        gg_qni,gg_qnr,gg_cwmr,do_radar)
               else
                ...
                if(mype==iope) then
                   ...
                   if( do_radar )then
                     call this%parallel_read_wrf_mass_step2(mype,iope,&
                          ps,u,v,tv,rh,cwmr,oz,w,qr,qs,qi,qg,dbz,qni,qnr,qnc, &
                          gg_ps,gg_tv,gg_u,gg_v,gg_rh,gg_w,gg_dbz,gg_qr,&
                          gg_qs,gg_qi,gg_qg,gg_qnc,gg_qni,gg_qnr,gg_cwmr,do_radar)
                   else
                     call this%parallel_read_wrf_mass_step2(mype,iope,&
                   ...
                   if( do_radar )then
                     call this%parallel_read_wrf_mass_step2(mype,iope,&
                          ps,u,v,tv,rh,cwmr,oz,w,qr,qs,qi,qg,dbz,qni,qnr,qnc,do_radar=do_radar)
                   else
                     call this%parallel_read_wrf_mass_step2(mype,iope,&
                          ps,u,v,tv,rh,cwmr,oz)
                   endif
  ...
  !
     ...
     if(allocated(gg_w)) deallocate(gg_w)
     if(allocated(gg_dbz)) deallocate(gg_dbz)
     if(allocated(gg_qr)) deallocate(gg_qr)
     if(allocated(gg_qs)) deallocate(gg_qs)
     if(allocated(gg_qi)) deallocate(gg_qi)
     if(allocated(gg_qg)) deallocate(gg_qg)
     if(allocated(gg_qnc)) deallocate(gg_qnc)
     if(allocated(gg_qni)) deallocate(gg_qni)
     if(allocated(gg_qnr)) deallocate(gg_qnr)
     if(allocated(gg_cwmr)) deallocate(gg_cwmr)
...
20 write(6,*) 'get_wrf_mass_ensperts_netcdf: read WRF-ARW ens failed ',n
  ...
  subroutine parallel_read_wrf_mass_step1(this,filename,gg_ps,gg_tv,gg_u,gg_v,gg_rh,gg_w,&
                                                        gg_dbz,gg_qr,gg_qs,gg_qi,gg_qg,gg_qnc,&
                                                        gg_qni,gg_qnr,gg_cwmr,do_radar)
  !$$$  subprogram documentation block
  !                .      .    .                                       .
  ...
  ! Declare passed variables
      ...
      real(r_kind),dimension(grd_ens%nlat,grd_ens%nlon,grd_ens%nsig), optional :: gg_w,&
                                                        gg_dbz,gg_qr,gg_qs,gg_qi,gg_qg,gg_qnc,&
                                                        gg_qni,gg_qnr,gg_cwmr

      logical,optional :: do_radar
  ...
  !   Declare local variables
      ...
      integer(i_kind):: nx,ny,nz,i,j,k,d_max,file_id,var_id,ndim,ivar
...
30 format(I0,': read ',A)
  ...
  subroutine parallel_read_wrf_mass_step2(this,mype,iope, &
       g_ps,g_u,g_v,g_tv,g_rh,g_cwmr,g_oz,g_w,g_qr,g_qs,g_qi,&
       g_qg,g_dbz,g_qni,g_qnr,g_qnc, &
       gg_ps,gg_tv,gg_u,gg_v,gg_rh,gg_w,gg_dbz,gg_qr,&
       gg_qs,gg_qi,gg_qg,gg_qnc,gg_qni,gg_qnr,gg_cwmr,do_radar)
  ...
  ! Declare passed variables
      ...
      real(r_kind),dimension(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig),intent(out),optional::&
                                  g_w,g_qr,g_qs,g_qi,g_qg,g_dbz,g_qni,g_qnr,g_qnc 
      ...
      real(r_kind),optional,dimension(grd_ens%nlat,grd_ens%nlon,grd_ens%nsig) :: &
           gg_w,gg_dbz,gg_qr,gg_qs,gg_qi,gg_qg,gg_qnc,gg_qni,gg_qnr,gg_cwmr
  ...
  ! then TV,U,V,RH
    do k=1,grd_ens%nsig
       ...
       if( do_radar )then
         ...
         if (mype==iope) call this%fill_regional_2d(gg_qnr(1,1,k),wrk_send_2d)
         call mpi_scatterv(wrk_send_2d,grd_ens%ijn_s,grd_ens%displs_s,mpi_rtype,&
         g_qnr(1,1,k),grd_ens%ijn_s(mype+1),mpi_rtype,iope,mpi_comm_world,ierror)
         if (mype==iope) call this%fill_regional_2d(gg_qnc(1,1,k),wrk_send_2d)
         call mpi_scatterv(wrk_send_2d,grd_ens%ijn_s,grd_ens%displs_s,mpi_rtype,&
         g_qnc(1,1,k),grd_ens%ijn_s(mype+1),mpi_rtype,iope,mpi_comm_world,ierror)
         if (mype==iope) call this%fill_regional_2d(gg_qni(1,1,k),wrk_send_2d)
         call mpi_scatterv(wrk_send_2d,grd_ens%ijn_s,grd_ens%displs_s,mpi_rtype,&
         g_qni(1,1,k),grd_ens%ijn_s(mype+1),mpi_rtype,iope,mpi_comm_world,ierror)
         if (mype==iope) call this%fill_regional_2d(gg_cwmr(1,1,k),wrk_send_2d)
         call mpi_scatterv(wrk_send_2d,grd_ens%ijn_s,grd_ens%displs_s,mpi_rtype,&
         g_cwmr(1,1,k),grd_ens%ijn_s(mype+1),mpi_rtype,iope,mpi_comm_world,ierror)
       end if
    enddo
  ! for now, don't do anything with oz, cwmr
    g_oz = 0.
    if( .not. do_radar  )g_cwmr = 0.
