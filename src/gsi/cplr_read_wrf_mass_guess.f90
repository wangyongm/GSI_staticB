...
!                                    read qnc
  ...
  ! other internal variables
    ...
    integer(i_kind) :: i_mask,i_mask1, kmask,kmask1 ! ywang
    ...
    real(r_kind), pointer :: ges_mask(:,:,:)=>NULL()  ! ywang
    real(r_kind), pointer :: ges_mask1(:,:,:)=>NULL() ! ywang
...
!    The 9 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,qni,qnc,tt
       if(l_hydrometeor_bkio .or.n_actual_clouds>0) num_mass_fields=num_mass_fields+9*lm+2
       ...
       if( if_cs_staticB ) num_mass_fields = num_mass_fields + lm
       if( adaptive_hybrid ) num_mass_fields = num_mass_fields + lm 
  ...
  ! for cloud analysis
       ...
       if(l_hydrometeor_bkio .or. n_actual_clouds>0) jsig_skip(i)=0 ! number of files to skip before getting to psfc
       igtype(i)=1
  ...
  ! hydrometeors
             ...
             if(if_cs_staticB) kmask=kmask+1
             if(adaptive_hybrid) kmask1=kmask1+1
             if ( laeroana_gocart ) then
                if ( n_gocart_var > 0 ) then
                   do iv = 1, n_gocart_var
                      kchem(iv) = kchem(iv)+1
  ...
  !                    ges_tten(j,i,k,it) = real(all_loc(j,i,ktt),r_kind)
                   ...
                   if(if_cs_staticB) then
                     ges_mask(j,i,k) = all_loc(j,i,kmask)
                   end if
                   if(adaptive_hybrid)then
                     ges_mask1(j,i,k) = all_loc(j,i,kmask1)
                   end if
                   if ( laeroana_gocart ) then
