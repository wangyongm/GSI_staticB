...
contains  
  ...
  !$$$
    ...
    use obsmod, only   : if_model_dbz,if_cs_staticB
  ...
  ! rmse stuff
    ...
    real(r_single),allocatable::field3u(:,:,:),field3v(:,:,:),field1a(:),field3tmp(:,:,:)
  ...
  !-------------  get grid info
       ...
       allocate(field3tmp(nlon_regional,nlat_regional,nsig_regional)) ! ywang
       if(w_exist) then
...
!      rmse_var='LANDMASK'
       ...
       if(l_hydrometeor_bkio .or. n_actual_clouds>0) then
          rmse_var='QCLOUD'
          call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
               start_index,end_index, WrfType, ierr    )
          if(print_verbose)then
...
! ywang >
       if( if_cs_staticB ) then
          ...
          call ext_ncd_open_for_read( trim(flnm1), 0, 0, "", dh2, Status)
          if ( Status /= 0 )then
             write(6,*)'CONVERT_NETCDF_MASS:  problem with flnm1 = ',&
                  trim(flnm1),', Status = ', Status
             call stop2(74)
          ...
          call ext_ncd_read_field(dh2,DateStr1,TRIM(rmse_var),              &
               field3tmp,WRF_REAL,0,0,0,ordering,           &
               staggering, dimnames ,               &
               start_index,end_index,               & !dom
               start_index,end_index,               & !mem
               start_index,end_index,               & !pat
          ...
          do k=1,nsig_regional
           do i = 1, nlon_regional
            do j = 1, nlat_regional
              if( field3(i,j,k) > 90.0 )then
                   field3(i,j,k) = 1.0
              else if ( field3(i,j,k) < -90.0 )then
                   field3(i,j,k) = -1.0
              else
                   field3(i,j,k) = 0.0
              end if
              if ( field3tmp(i,j,k) > 90.0 )then
                   field3tmp(i,j,k) = 1.0
              else
                   field3tmp(i,j,k) = 0.0
              end if
          ...
          do k=1,nsig_regional
             write(6,*)' k,max,min,mid masker=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
                      field3(nlon_regional/2,nlat_regional/2,k)
             write(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional) ! mask
          end do

          do k=1,nsig_regional
             write(6,*)' k,max,min,mid maskerH=',k,maxval(field3tmp(:,:,k)),minval(field3tmp(:,:,k)), &
                      field3tmp(nlon_regional/2,nlat_regional/2,k)
             write(iunit)((field3tmp(i,j,k),i=1,nlon_regional),j=1,nlat_regional) ! mask_H
          end do
