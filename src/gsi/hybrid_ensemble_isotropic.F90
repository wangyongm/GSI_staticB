...
! Declare local variables
  ...
  real(r_kind),dimension(:,:,:),pointer::ges_mask1=>NULL() ! ywang
  real(r_kind)    :: sqrt_beta_s_temp ! ywang
  ...
  call gsi_bundlegetpointer (gsi_metguess_bundle(1),'maskh',ges_mask1,istatus) ! ywang
...
! Declare local variables
  ...
  integer(i_kind) :: i,j,k,ii,nn,istatus
  ! ywang
  real(r_kind),dimension(:,:,:),pointer::ges_mask1=>NULL()
        ...
        do nn=1,n_ens
           do k=1,nsig
              do i=1,grd_ens%lat2
                 ...
                 grady%aens(ii,nn)%r3(1)%q(i,j,k) = sqrt_beta_e_temp*grady%aens(ii,nn)%r3(1)%q(i,j,k)
              enddo
           enddo
        enddo
     enddo
...
! Declare local variables
  character(len=*),parameter::myname_=myname//'*sqrt_beta_e_mult'
  integer(i_kind) :: i,j,k,nn,istatus
  ! ywang
  real(r_kind),dimension(:,:,:),pointer::ges_mask1=>NULL()
  real(r_kind)    :: sqrt_beta_e_temp
     ...
     do nn=1,n_ens
        do k=1,nsig
           do i=1,grd_ens%lat2
              ...
              aens(nn)%r3(1)%q(i,j,k) = sqrt_beta_e_temp*aens(nn)%r3(1)%q(i,j,k)
           enddo
        enddo
     enddo
  enddo
