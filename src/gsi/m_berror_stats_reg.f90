...
! ywang >
      public :: berror_get_dims_reg_extra
      public :: berror_read_bal_reg_extra
      public :: berror_read_wgt_reg_extra
! ywang <
...
! Read dimension of stats file
  ...
  open(inerr,file=berror_stats,form='unformatted',status='old',convert="little_endian")
  rewind inerr
...
! ywang >
    ...
    subroutine berror_get_dims_reg_extra(msig,mlat,num_bins2d,unit)

      implicit none

      integer(i_kind)         ,intent(  out) :: msig  ! dimension of levels
      integer(i_kind)         ,intent(  out) :: mlat  ! dimension of latitudes
      integer(i_kind)         ,intent(  out) :: num_bins2d ! dimension of category
      integer(i_kind),optional,intent(in   ) :: unit  ! logical unit [22]
...
! Read dimension of stats file
  ...
  open(inerr,file=berror_stats,form='unformatted',status='old',convert="little_endian")
  rewind inerr
  read(inerr) msig,mlat,num_bins2d
  close(inerr)
end subroutine berror_get_dims_reg_extra
...
! ywang >
    subroutine berror_read_bal_reg_extra(msig,mlat,num_bins2d,agvi,bvi,wgvi,evi2,evi3,mype,unit)
      use kinds,only : r_single
      use guess_grids, only:  ges_psfcavg,ges_prslavg

      implicit none

      integer(i_kind)                              ,intent(in   ) :: msig,mlat,num_bins2d
      real(r_kind),dimension(0:mlat+1,nsig,nsig),intent(  out) :: agvi
      real(r_kind),dimension(0:mlat+1,nsig)     ,intent(  out) :: wgvi
      real(r_kind),dimension(0:mlat+1,nsig)     ,intent(  out) :: bvi
      real(r_kind),dimension(15,0:mlat+1,num_bins2d,nsig)     ,intent(  out) :: evi2
      real(r_kind),dimension(60,0:mlat+1,num_bins2d,nsig,nsig),intent(  out) :: evi3
      integer(i_kind)                              ,intent(in   ) :: mype  ! "my" processor ID
      integer(i_kind),optional                     ,intent(in   ) :: unit ! an alternative unit
...
!   workspaces/variables for data not returned
  ...
  integer(i_kind) k,i,m,j,m1,l1,l,ivar
  integer(i_kind):: nsigstat,nlatstat,num_bins2d1
  integer(i_kind):: inerr
  ...
  real(r_single),dimension(:,:,:,:),allocatable  :: extravi2
  real(r_single),dimension(:,:,:,:,:),allocatable :: extravi3
...
!   Read header.
    rewind inerr
    read(inerr) nsigstat,nlatstat,num_bins2d1
   ...
   if( if_exist )then
    ...
    allocate ( extravi2(extra_var2num,0:mlat+1,num_bins2d,1:msig) )
    allocate ( extravi3(extra_var3num,0:mlat+1,num_bins2d,1:msig,1:msig) )
...
!   Read background error file to get balance variables
    read(inerr)clat_avn,(sigma_avn(k),k=1,msig)
    read(inerr)agv_avn,bv_avn,wgv_avn ,extravi2,extravi3
    close(inerr)

!print*,'extravi3',extravi3(1:extra_var3num,2,1,1)
...
!   Load agv wgv bv
    do k=1,nsig
       ...
       do i=1,mlat
          ...
          do ivar = 1, extra_var2num
            evi2(ivar,i,:,k) = extravi2(ivar,i,:,m)*coef1(k)+extravi2(ivar,i,:,m1)*coef2(k)
          end do
       enddo
       ...
       do j=1,nsig
          ...
          do i=1,mlat
             ...
             do ivar=1,extra_var3num
               evi3(ivar,i,:,j,k)=(extravi3(ivar,i,:,l,m)*coef1(j)+extravi3(ivar,i,:,l1,m)*coef2(j))*coef1(k) &
                               +(extravi3(ivar,i,:,l,m1)*coef1(j)+extravi3(ivar,i,:,l1,m1)*coef2(j))*coef2(k)
             end do
          enddo
       enddo
    enddo
    ...
    evi2(:,0,:,:)=evi2(:,1,:,:)
    evi2(:,mlat+1,:,:)=evi2(:,mlat,:,:)
    evi3(:,0,:,:,:) = evi3(:,1,:,:,:)
    evi3(:,mlat+1,:,:,:) = evi3(:,mlat,:,:,:)

    deallocate (agv_avn,bv_avn,wgv_avn,clat_avn,sigma_avn,rlsigo)
    deallocate (extravi3,extravi2)
    return
end subroutine berror_read_bal_reg_extra
    ...
    subroutine berror_read_wgt_reg_extra(msig,mlat,num_bins2d,corz,corp,hwll,hwllp,vz,rlsig,varq,qoption,varcw,cwoption,mype,unit)
      ...
      real(r_kind),dimension(0:mlat+1,1:nsig,num_bins2d,1:nc3d), intent(inout):: hwll
      real(r_kind),dimension(0:mlat+1,num_bins2d,nvars-nc3d)   , intent(inout):: hwllp
      real(r_kind),dimension(nsig,0:mlat+1,num_bins2d,1:nc3d),intent(inout):: vz
...
!  workspace variables not returned
  real(r_single),dimension(:),allocatable::  clat_avn,sigma_avn
  real(r_single),dimension(:,:),allocatable::  bv_avn,wgv_avn
  real(r_single),dimension(:,:,:),allocatable:: agv_avn,corqq_avn
  real(r_single),dimension(:,:,:),allocatable:: corz_avn,hwll_avn,vztdq_avn

  real(r_single),dimension(1:mlat,msig,num_bins2d,nrf):: corz_tmp
  real(r_single),dimension(0:mlat+1,msig,num_bins2d,nrf):: hwll_tmp
  real(r_single),dimension(msig,0:mlat+1,num_bins2d,nrf):: vz_tmp
  ...
  integer(i_kind) :: nrf3_sfwter,nrf3_vpwter,nrf3_dbz,nrf3_qni,nrf3_qnc,nrf3_qnr
  integer(i_kind) :: inerr,istat
  integer(i_kind) :: nsigstat,nlatstat,isig,num_bins2d1
  ...
  real(r_single),dimension(20,0:mlat+1,num_bins2d,msig)  :: extravi2
  real(r_single),dimension(100,0:mlat+1,num_bins2d,msig,msig):: extravi3
...
! Read header.
  ...
  read(inerr) nsigstat,nlatstat,num_bins2d1
   ...
   if( if_exist )then
    ...
    !allocate ( extravi2(extra_var2num,0:mlat+1,1:msig) )
    !allocate ( extravi3(extra_var3num,0:mlat+1,1:msig,1:msig) )
...
! Read background error file to get balance variables
  read(inerr)clat_avn,(sigma_avn(k),k=1,msig)
  read(inerr)agv_avn,bv_avn,wgv_avn,extravi2(1:extra_var2num,0:mlat+1,num_bins2d,1:msig),extravi3(1:extra_var3num,0:mlat+1,num_bins2d,1:msig,1:msig)
...
! Read amplitudes
  ...
  read: do
     ...
     if (istat /= 0) exit
     allocate ( corz_avn(1:mlat,1:isig,1:num_bins2d) )
     allocate ( hwll_avn(0:mlat+1,1:isig,1:num_bins2d) )
     allocate ( vztdq_avn(1:isig,0:mlat+1,1:num_bins2d) )
     if (var/='q' .or. (var=='cw' .and. cwoption==2)) then
        read(inerr) corz_avn
     else
        allocate ( corqq_avn(1:mlat,1:isig,1:num_bins2d) )
...
! Get control variable indexes
  ...
  nrf3_qni =getindex(cvars3d,'qni')
  nrf3_qnr =getindex(cvars3d,'qnr')
  nrf3_qnc =getindex(cvars3d,'qnc')
  ...
  if(nrf3_q>0 .and. qoption==2)then
     do k=1,nsig
        do j=1,mlat
           varq(j,k)=min(max(real(corz(j,k,1,nrf3_q),r_kind),0.0015_r_kind),one)
           corz(j,k,:,nrf3_q)=one
        enddo
     enddo
  endif
...
!    hwll:,:,nrf3_oz)=400000.0_r_kind
  ...
  if (nrf3_qni>0) then
     corz(:,:,:,nrf3_qni)=one
     hwll(:,:,:,nrf3_qni)=hwll(:,:,:,nrf3_dbz)
     vz(:,:,:,nrf3_qni)=vz(:,:,:,nrf3_dbz)
  end if

  if (nrf3_qnc>0) then
     corz(:,:,:,nrf3_qnc)=one
     hwll(:,:,:,nrf3_qnc)=hwll(:,:,:,nrf3_dbz)
     vz(:,:,:,nrf3_qnc)=vz(:,:,:,nrf3_dbz)
  end if

  if (nrf3_qnr>0) then
     corz(:,:,:,nrf3_qnr)=one
     hwll(:,:,:,nrf3_qnr)=hwll(:,:,:,nrf3_dbz)
     vz(:,:,:,nrf3_qnr)=vz(:,:,:,nrf3_dbz)
  end if
     ...
     if (cwoption==2) then
        do k=1,nsig
           if (ges_prslavg(k)>15.0_r_kind) then
              do j=1,mlat
                 varcw(j,k)=max(real(corz(j,k,1,nrf3_cw),r_kind),zero)
