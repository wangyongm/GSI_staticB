...
!$$$
  ...
  use m_berror_stats_reg, only: berror_get_dims_reg_extra,berror_read_wgt_reg_extra ! ywang
  ...
  use obsmod, only: if_cs_staticB ! ywang
...
! Declare local variables
  ...
  real(r_kind),allocatable,dimension(:,:,:,:):: dsv ! ywang
  real(r_kind),dimension(lon2,llmin:llmax):: dsvs

  real(r_kind),allocatable,dimension(:,:,:):: corp, hwllp      ! ywang
  real(r_kind),allocatable,dimension(:,:,:,:):: corz, hwll, vz ! ywang
  ...
  real(r_kind),dimension(:,:,:),pointer::ges_mask=>NULL() ! ywang
...
! Allocate arrays in stats file
  ...
  allocate ( corp(1:mlat,1:num_bins2d,nc2d) )
...
! note:  parameter vs needs to be inverted
  ...
  do n=1,nc3d
     if(n==nrf3_oz .and. regional_ozone)then   ! spetial treament for ozone variance
        loc=nrf3_loc(n)
        vz(:,:,:,n)=1.5_r_kind   ! ozone vertical scale fixed ! ywang
